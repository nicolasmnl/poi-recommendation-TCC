...
word2vec = Word2Vec.load(f'./geo2vec_embeddings/{embeddings}')
model_lines = Word2Vec.load(f'./db_parser/geographic/model/{lines}')
model_points = Word2Vec.load(f'./db_parser/geographic/model/{points}')
model_roads = Word2Vec.load(f'./db_parser/geographic/model/{roads}')
model_polygons = Word2Vec.load(f'./db_parser/geographic/model/{polygons}')
...
def get_custom_embeddings(cat_id):
    ...
    embed_lines = model_lines.wv[category]
    embed_points = model_points.wv[category]
    embed_roads = model_roads.wv[category]
    embed_polygons = model_polygons.wv[category]
...
def train(args):
    ...
    args.seq_input_embed = args.poi_embed_dim + args.user_embed_dim + args.time_embed_dim + args.cat_embed_dim + 140
